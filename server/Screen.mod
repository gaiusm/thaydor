...
BEGIN
   ...
   'q'      : StrIO.WriteString ('Check Status ') |
   'x'      : StrIO.WriteString ('Blow Dart    ') |
...
CONST
   Fifth = TicksPerSecond DIV 5 ;   (* 25 DIV 5.  *)
VAR
   ticks: CARDINAL ;
BEGIN
   ticks := GetTicks () ;
   ...
   NumberIO.WriteCard((ticks DIV TicksPerSecond) DIV 60, 0) ;
   Write(':') ;
   NumberIO.WriteCard((ticks DIV TicksPerSecond) MOD 60, 0) ;
   Write('.') ;
   NumberIO.WriteCard(((ticks MOD TicksPerSecond) DIV Fifth) * 2, 0) ;
   StrIO.WriteLn
END WriteTime ;
