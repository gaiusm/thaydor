...
FROM AdvMath IMPORT MaxNoOfTreasures, LowFreePool, HighFreePool,
                    ...
                    EndlessQuiver, BlowDart ;

FROM AdvUtil IMPORT InitialDisplay, ClosedToTimedDoor, TimedDoorToClosed ;
...
BEGIN
   ...
   THEN
      ...
      ELSE
         GetWriteAccessToPlayer ;
         DropTreasure1(p, TreasNo, ReDraw) ;
         ReleaseWriteAccessToPlayer
      END ;
      IF ReDraw
      THEN
         ...
         THEN
            ...
            THEN
               ...
               THEN
                  ...
                  WriteCommentLine2(p, Treasure[TreasNo].TreasureName) ;
                  ReleaseAccessToScreenNo( p ) ;
                  PutDownTreasure(p, r, TreasNo, x, y, ReDraw)
               ELSE
                  GetAccessToScreenNo(p) ;
                  WriteCommentLine1(p, 'thou canst') ;
                  DelCommentLine2(p) ;
   ...
   LOOP
      ...
      THEN
         ...
         FOR i := 0 TO MaxNoOfPlayers DO
            IF i IN SlainP
            THEN
               Dead (i, PlayerPulled, RoomOfExplosion)
            END
         END
      END ;
      ...
      THEN
         WITH qHead^ DO
            ...
            THEN
               GetWriteAccessToDoor ;
               WITH qHead^ DO
                  success := TimedDoorToClosed (Rm, x, y, dir)
               END ;
               ReleaseWriteAccessToDoor
...
PROCEDURE ActivateGoggles (p: CARDINAL) ;
BEGIN

END ActivateGoggles ;


PROCEDURE DeActivateGoggles (p: CARDINAL) ;
BEGIN

END DeActivateGoggles ;
...
BEGIN
   ...
   initTreasure (EndlessQuiver, 1, unused) ;
   initTreasure (BlowDart,      0, unused) ;
   initTreasure (MagicGoggles,  1, unused) ;
